...
private:
	SDL_Surface *screen;
    int pixels[W][H];
	static const int constMaxNScreenObjects = 512;
	ScreenObject *screenObjects[constMaxNScreenObjects];
	int nScreenObjects;
public:
	int Initialize() {
		nScreenObjects = 0;
...
*/
	...
	void Redraw() {
//		Clean();
		for (int i = 0; i < nScreenObjects; i++)
			screenObjects[i]->Redraw();
		ShowScreen();
	}
	void AddScreenObject(ScreenObject *screenObject) {
		screenObjects[nScreenObjects++] = screenObject;
	}
	void DeleteScreenObject(ScreenObject *screenObject) {
