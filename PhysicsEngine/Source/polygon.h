...
//		if (paintNeed)
//			graphics->DrawLine((int)r.x, (int)r.y, (int)(r.x + p.x / 100), (int)(r.y + p.y / 100), RGB(255, 0, 0));
	}
	...
	virtual bool PointInside(Vector2D p) = 0;
};
...
public:
	...
	void Redraw() {
		...
		for (int i = 0; i < nPoints; i++)
			graphics->DrawLine((int)worldPoints[i].x, (int)worldPoints[i].y, (int)worldPoints[(i + 1) % nPoints].x, (int)worldPoints[(i + 1) % nPoints].y, RGB(0, 0, 255));
	}
...
public :
	...
	void GetProjection(Vector2D normal, double &minI, double &maxI) {
		minI = maxI = transformToWorld(Vector2D(0, 0, 1)) * normal;
	...
	void Redraw() {
		graphics->DrawCircle((int)centerPosition.x, (int)centerPosition.y, (int)radius, RGB(0, 255, 0));
		graphics->DrawLine((int)centerPosition.x, (int)centerPosition.y, (int)(centerPosition.x + radius * cos(rotationAngle)), (int)(centerPosition.y + radius * sin(rotationAngle)), RGB(0, 255, 0));
	}
	bool PointInside(Vector2D p) {
//		p = transformToWorldInverse(p);
		return (p - centerPosition).GetLength() <= radius + eps;
	}
