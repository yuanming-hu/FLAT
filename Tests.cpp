	...
	if (input.keyboard.IsPressed(GLFW_KEY_LCTRL)) {
		...
		for (int i = 0; i < 20; i++)
			for (int j = 0; j < 25; j++) {
				...
				box->SetPosition(Vector2D(100 + width * j * 1.05, 100 + width * i * 1.05, 1));
...
void Game::Test2() {
	...
	for (int i = 0; i < n; i++) {
		...
		DistanceConstraint *con = new DistanceConstraint(obj, Vector2D(0, 0, 0), physics.worldBox, Vector2D(x, 400, 0));
		con->color = cir->color;
...
void Game::Test3() {
	...
	for (int i = 0; i < n; i++) {
		...
		boxes[i]->SetPosition(Vector2D(screenWidth / 2, screenHeight - 30 - i * 7, 1));
	...
	for (int i = 0; i < n - 1; i++) {
		Spring *spring0 = new Spring(boxes[i], Vector2D(-23, 0, 1), boxes[i + 1], Vector2D(-23, 0, 1), 8000, 5);
		Spring *spring1 = new Spring(boxes[i], Vector2D(23, 0, 1), boxes[i + 1], Vector2D(23, 0, 1), 8000, 5);
...
void Game::Test4() {
	physics.SetAsDefaultWorld();
	RGB3f color0 = RGB3f::RandomBrightColor(), color1 = RGB3f::RandomBrightColor();
...
//	Vector2D p = mainObj->GetTransformToWorldInverse()(Vector2D(centerX, centerY, 1));
	DistanceConstraint *constraint = new DistanceConstraint(physics.worldBox, Vector2D(centerX, centerY, 0), 
		mainObj, Vector2D(centerX, centerY, 1) - mainObj->GetPosition());
...
//	subObj->GetShape(0)->density = 0.3;
	...
	physics.AddConstantConstraint(
		new DistanceConstraint(mainObj, Vector2D(centerX - R, centerY, 1) - mainObj->GetPosition(), 
	...
	physics.AddObject(subObj);
	physics.AddConstantConstraint(
		new DistanceConstraint(mainObj, Vector2D(centerX, centerY - R, 1) - mainObj->GetPosition(), 
	...
	physics.AddConstantConstraint(new DistanceConstraint(cir, Vector2D(0, 0, 0), cir->centroidPosition));
	...
	physics.AddConstantConstraint(new DistanceConstraint(cir, Vector2D(0, 70, 0), box, Vector2D(0, 0, 0)));
	*/
}
...
/*	physics.DeleteAllShapes();
	...
	double R = 9.0;
	Circle *cir[3][20];
	RGB3f color = RGB3f::RandomBrightColor(), springColor = RGB3f::RandomBrightColor();
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m - i; j++) {
			cir[i][j] = Circle::GenerateCircle(R);
			cir[i][j]->color = color;
			cir[i][j]->SetCenterPosition(Vector2D(100 + R * 4 * j + R * i * 2, 100 + R * 4 * i, 1));
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m - i; j++) {
			if (j + 1 < m - i) {
				Spring *spring = new Spring(cir[i][j], Vector2D(0, 0, 1), cir[i][j + 1], Vector2D(0, 0, 1), strength);
				spring->color = springColor;
				physics.AddForce(spring);
			}
			if (i + 1 < n) {
				if (j > 0) {
					Spring *spring = new Spring(cir[i][j], Vector2D(0, 0, 1), cir[i + 1][j - 1], Vector2D(0, 0, 1), strength);
					spring->color = springColor;
					physics.AddForce(spring);
				}
				if (j < m - i - 1) {
					Spring *spring = new Spring(cir[i][j], Vector2D(0, 0, 1), cir[i + 1][j], Vector2D(0, 0, 1), strength);
					spring->color = springColor;
					physics.AddForce(spring);
				}
			}
